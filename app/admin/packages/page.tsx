...
class SimpleErrorBoundary extends React.Component<any, { hasError: boolean, error: any }> {
  ...
  render() {
    if (this.state.hasError) {
      return <div style={{ background: '#fee', color: '#900', padding: 16, fontWeight: 700 }}>ERRORE JS: {String(this.state.error && this.state.error.message)}</div>;
    }
...
export default function PackagesManagementPage() {
  console.log('PackagesManagementPage render start');
