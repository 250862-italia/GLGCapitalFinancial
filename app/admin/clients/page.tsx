...
export default function ClientsPage() {
  ...
  const filteredClients = (clients || []).filter(client => {
    // Controllo di sicurezza per evitare errori di rendering
    if (!client || !client.first_name || !client.last_name) {
      console.warn('⚠️ Cliente con dati incompleti:', client);
      return false;
    }
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredClients.length > 0 ? (
                      filteredClients.map((client) => {
                        // Controllo di sicurezza per ogni cliente
                        if (!client || !client.id || !client.first_name || !client.last_name) {
                          console.warn('⚠️ Cliente con dati incompleti nella tabella:', client);
                          return null;
                        }
                        ...
                        return (
                          <tr key={client.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getRiskProfileColor(client.risk_profile)}`}>
