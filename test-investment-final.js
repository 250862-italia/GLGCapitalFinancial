...
console.log('🔍 Test Finale Investment Process');
...
async function testFinalInvestmentProcess() {
  try {
    ...
    if (allClients.length > 0) {
      console.log('   Esempi:', allClients.map(c => `${c.account_holder} (${c.status})`));
    }
    ...
    console.log('   Cliente:', testClient.account_holder);
    ...
    const testInvestment = {
      ...
      end_date: new Date(Date.now() + (testPackage.duration_days || 30) * 24 * 60 * 60 * 1000).toISOString(),
